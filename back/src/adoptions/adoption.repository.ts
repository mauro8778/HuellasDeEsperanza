...
export class AdoptionRepository {
    ...
    if (!shelter) {
        throw new BadRequestException(`Refugio no encontrado para la mascota especificada`);
    }
        ...
        await this.mailservice.sendPostulacion(shelter.name, pet.name, user.name, user.email);

        this.scheduleAdoptionCheck(adop.id, user.name, shelter.name, pet.name, user.email);
   ...
   scheduleAdoptionCheck(adoptionId: string, username: string, sheltername: string, pet: string, userEmail: string) {
    ...
    const task = cron.schedule('0 0 */78 * * *', async () => {
      ...
      if (adoption && !adoption.isActive) {
        const subject = 'Solicitud de adopción rechazada en Huellas de Esperanza';
        const text = `Estimado/a ${username},\n\nLamentamos informarte que el refugio ${sheltername} no ha aceptado tu solicitud de adopción para ${pet}. Te invitamos a visitar nuestra página y ponerte en contacto con el refugio a través del chat para resolver cualquier duda o inquietud.\n\nCon cariño,\nEl Equipo de Huellas de Esperanza`;
        const html = `<div style="border: 2px solid #ff3366; padding: 20px; background: #ffffff; border-radius: 15px; text-align: center;">
            ...
            <p style="color: #000; font-size: 16px; margin-bottom: 20px;">Lamentamos informarte que el refugio <strong>${sheltername}</strong> no ha aceptado tu solicitud de adopción para <strong>${pet}</strong>.</p>
        ...
        this.logger.log(`Enviando correo de rechazo a ${userEmail} para la adopción de ${pet}`);
      }
    }, {
      scheduled: true,
